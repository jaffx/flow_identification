...
class MSFINet(torch.nn.Module):
    def __init__(self, num_classes=7, head1=None, head2=None, fusion=None):
        ...
        # 特征融合模块
        if fusion is None:
            self.fusion = Fusion.ConvFusion(512, 512, 512)
        else:
            self.fusion = fusion
...
class MSFINet_FullTrunk(torch.nn.Module):

    def __init__(self, num_classes=7):
        super(MSFINet_FullTrunk, self).__init__()
        self.head1 = Res1D.resnet1d34(include_top=False)
        # 特征融合模块
        self.fusion = Fusion.ConvFusion(512, 512, 512)
        self.fusionClassifier = Module.Classifier(512, num_classes)
...
class MSFINet_HalfBranch(torch.nn.Module):

    def __init__(self, num_classes=7):
        super(MSFINet_HalfBranch, self).__init__()
        self.head1 = Res1D.resnet1d34(include_top=False)
        self.head2 = Res1D.resnet1d34(include_top=False)
        # 特征融合模块
        self.fusion = Fusion.ConvFusion(512, 512, 512)
