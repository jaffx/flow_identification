...
out_pressure_path = f"/Users/lyn/codes/python/Flow_Identification/Dataset/v4/Pressure/v4_DiffPressure_IDX{IDX1}-{IDX2}_Simple_A"
        ...
        with open(filePath) as rfp, open(outFileName, "w+") as wfp:
            ...
            for line in content:
                ...
                if len(items) != 5:
                    ...
                    continue
                if not isfloat(items[IDX1]) or not isfloat(items[IDX2]):
                    continue
                target_line = f"{float(items[IDX1]) - float(items[IDX2]):.4f}\n"
                lineNum += 1
