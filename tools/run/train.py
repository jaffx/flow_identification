...
from tools.utils.setting import *
from tools.utils.dataset import get_dataset_path


def train(setting: train_setting, model):
    # 定义训练设备
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("using {} device.".format(device))

    # 自定义训练参数
    data_length = setting.data_length
    sampling_step = setting.sampling_step
    batch_size = setting.batch_size
    epoch_num = setting.epoch_num
    dataset_path = get_dataset_path(setting.dataset, setting.device_name)
